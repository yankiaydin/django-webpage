...
def edit_profile(request, user):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            persona = Profile(user = u1,location= form.cleaned_data.get("location"), web_page= form.cleaned_data.get("web_page"),
                              job= form.cleaned_data.get("job"), bio= form.cleaned_data.get("bio"), profile_pic = form.cleaned_data.get("profile_pic"))
            persona.save()
            if persona.job == "Architect":
                return HttpResponseRedirect(reverse("member:architect_profile", args=[user]))
            elif persona.job == "Constructor":
                return HttpResponseRedirect(reverse("member:const_profile", args=[user]))
